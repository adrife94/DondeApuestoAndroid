...
public class CasinoMadrid {
    public void parseadorMadrid(String casa,String liga,String url ) {


        try {

            Document madrid = Jsoup.connect(url).get();  //obtenemos la informacion de la pagina que le decimos
            Elements elementosEquip = madrid.select("div [class=snc-bets-item snc-bets-item-withoutactor]");//.not("span[class=top-selns]"); Si ponemos elements es para muchos, element para uno. Y con lo que esta entrecomillado filtramos.
            Elements elementosCuot = elementosEquip.select("div [class=odds-choice-label]");
            Elements elementosCuotass = elementosEquip.select("div [class=snc-odd-odd block-fixed-right-for-ellipsis ]");
